...
export class StartTask {
    ...
    async run(stop: { state?: boolean } = {}) {
        ...
        if (process.env.DEPLOY_PLATFORM === 'heroku') {
            ...
            return await this.execService.call(`nodemon --watch '${cwd}/src/**/*.ts' ${this.quiet ? '--quiet' : ''}  --ignore '${schema ? schema.introspectionOutputFolder : ''}/' --ignore '${cwd}/src/**/*.spec.ts' --exec '${this.config} && npm run lint && ${sleep} ts-node' ${customPathExists ? `${cwd}/${customPathExists ? customPath : 'index.ts'}` : `${cwd}/src/main.ts` }  ${this.verbose}`);
        }
    }
    extendConfig(config) {
