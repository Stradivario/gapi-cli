...
export class StartTask {
    ...
    async run(stop: { state?: boolean } = {}) {
        ...
        if (this.argsService.args.toString().includes('--docker')) {
            return await this.execService.call(`${this.config} && pm2-docker process.yml --only APP`);
        } else if (this.argsService.args.toString().includes('--pm2')) {
            if (!stop.state) {
                return await this.execService.call(`${this.config} && pm2 stop process.yml`);
            } else {
                return await this.execService.call(`${this.config} && pm2 start process.yml --only APP`);
            }
        }
